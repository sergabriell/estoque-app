...
class MovementsPage(ctk.CTkFrame):
    ...
    def build(self):
        ...
        # ── MONTAGEM DO HISTORY CONTAINER REVISADO COM O FRAME DE CABEÇALHO ──
        self.history_container = ModernCard(self)
        self.history_container.pack(fill="both", expand=True, padx=24, pady=(0, 24))

        # O Frame de Alinhamento Horizontal solicitado
        history_header_frame = ctk.CTkFrame(self.history_container, fg_color="transparent")
        history_header_frame.pack(fill="x", padx=18, pady=(18, 8))
        ...
        # Botão de Exportação acoplado ao Cabeçalho do Card
    ...
    def _product_names(self, product_list):
        return [f"{p.id} - {p.name} | Estoque: {p.stock}" for p in product_list]
    ...
    def on_supplier_change(self, value=None):
        ...
        self.product_combo.configure(values=names if names else ["Nenhum produto para este fornecedor"])
        self.product_combo.set(names[0] if names else "Nenhum produto para este fornecedor")
        self.update_stock_info()
    ...
    def on_type_change(self, value=None):
        ...
        if movement_type == "ENTRADA":
            self.save_button.configure(fg_color="#166534", hover_color="#14532D", text="Registrar entrada")
        else:
            self.save_button.configure(fg_color="#DC2626", hover_color="#B91C1C", text="Registrar saída")
        self.update_stock_info()

    def get_selected_product_id(self):
        product_text = self.product_combo.get()
        if " - " not in product_text: raise ValueError("Selecione um produto válido.")
    ...
    def save(self):
        try:
            if not self.all_products: raise ValueError("Cadastre um produto antes de registrar movimentações.")
            product_id = self.get_selected_product_id()
            movement_type = self.type_combo.get()
            ...
            self.movement_repository.create(product_id=product_id, movement_type=movement_type, quantity=quantity)
    ...
    def refresh_history(self):
        ...
        if not movements:
            ctk.CTkLabel(self.history_frame, text="Nenhuma movimentação registrada ainda.", text_color="#9CA3AF").pack(
                pady=20)
            return

        for movement in movements:
            row = ctk.CTkFrame(self.history_frame, corner_radius=14, fg_color="#0B1220")
            ...
            supplier_name = (movement.product.supplier.name if movement.product and movement.product.supplier else "—")

            ctk.CTkLabel(row, text=f"{type_label}  •  {product_name}", font=("Inter", 14, "bold"),
                         text_color=color).pack(side="left", padx=14, pady=12)
            ctk.CTkLabel(row, text=f"{symbol}{movement.quantity} unid.  |  Fornecedor: {supplier_name}",
                         text_color="#F9FAFB", font=("Inter", 13, "bold")).pack(side="left", padx=10)
            ctk.CTkLabel(row, text=movement.created_at.strftime("%d/%m/%Y %H:%M"), text_color="#9CA3AF").pack(
                side="right", padx=14)
    ...
    def export_to_csv(self):
        movements = self.movement_repository.list_all()
        if not movements:
            messagebox.showwarning("Aviso", "Nenhuma movimentação registrada para exportar.")
            return

        headers = ["ID", "Data/Hora", "Tipo de Movimento", "Produto SKU", "Produto Nome", "Quantidade", "Fornecedor"]

        rows = []
        for m in movements:
            product_name = m.product.name if m.product else "Produto removido"
            product_sku = m.product.sku if m.product else "—"
            supplier_name = (m.product.supplier.name if m.product and m.product.supplier else "—")
            date_str = m.created_at.strftime("%d/%m/%Y %H:%M")

            rows.append([
                m.id, date_str, m.movement_type, product_sku, product_name, m.quantity, supplier_name
            ])

        export_list_to_csv("historico_movimentacoes.csv", headers, rows)